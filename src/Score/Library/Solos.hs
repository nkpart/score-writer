...
buildPart ma = appEndo (execWriter ma) (Part [] NoRepeat)

bars :: Seq Beamed -> PartM ()
bars bs = fff (partBeams <>~ bs)

firstTime :: Seq Beamed -> PartM ()
firstTime x = fff (partRepeat %~ f)
  where f NoRepeat = Return (x, [])
        f Repeat = Return (x, [])
        f (Return (p,q)) = Return (p <> x, q)

secondTime :: Seq Beamed -> PartM ()
secondTime x = fff (partRepeat %~ f)
  where f NoRepeat = Return ([], x)
        f Repeat = Return ([], x)
        f (Return (p,q)) = Return (p, q <> x)

thenRepeat :: PartM ()
thenRepeat = fff (partRepeat .~ Repeat)

noRepeat :: PartM ()
noRepeat = fff (partRepeat .~ NoRepeat)
...
-- singles 4 r16 `zap` [flam, id, flam, flam]
-- h2h r16 [flam, id, flam, flam]
  ...
  where
    p1 = buildPart $
      do let commonStart = bars
         ...
         commonStart
         -- bars [dotCut (r16&flam<->l16<->r16&flam<->l16&flam), singles 4 r32 <-> singles 4 r32 & accentFirst
    ...
    --- PART 1
    -- p1 = Part (first2 <> [
    --               dotCut (r16&flam<->l16<->r16&flam<->l16&flam), singles 4 r32 <-> singles 4 r32 & accentFirst
    --               ,r8&accent.roll<->l8&accent.roll.endRoll, dotCut (r16&accent.endRoll.roll<-> r16&accent.endRoll<->l16&accent.roll<->l16&accent.endRoll)
    --               ] <> first2 <> ending2) Repeat
    --   where first2 =
    --           [r16 & flam <-> ldrag <-> dotCut (l16 <-> r16 & accent), dotCut (l16<->l16<->r16&drag<->r16&accent)
    --           ,dotCut (l16<->r16<->l16&drag<->l16 & accent), singles 4 r32 <-> dotCut (r16 & accent <-> l16)
    --           ]
    ...
    --- PART 2
    p2 = Part (first2 <> [
                  r32&flam<->l32<->r32<->r32<->dotCut (l16<->l16&flam), singles 4 r32 <-> r8&accent.roll
                  , dotCut(l16&endRoll.roll.accent<->l16&endRoll.accent)<->singles 4 r32, r8&accent<->dotCut(r16&flam<->r16&flam)
                         ]) (Return (first2 <> ending2, return2 <> ending2))
     where first2 =
             let b1 = [ r32&flam <-> l32 <-> r32 <-> r32 <-> dotCut (l16 <-> l16 & flam) , dotCut (r16 <-> r16 & flam <-> l16 <-> l16 & flam )  ]
              in b1 <> fmap swapHands b1
           return2 =
                     -- the start of this movement should be dot/cut in both rounded and swung
             let v = r16&P.dot.accent<->l16&P.cut.accent<->singles 4 r32&accentN 3<->singles 4 r32&accentN 0<->singles 4 r32&accentN 0
              in [v,v]

    --- PART 3
    p3 = Part (first2 <> end2' <> first2 <> ending2) Repeat
      where first2 =
    ...
    --- PART 4
    p4 = Part firstLine (Return (second2 <> ending2, return2 <> ending2 ))
      where firstLine = [
            ...
            second2 = twice [r32&flam<->l32<->r32<->r32<->triplet (l16<->r16<->l16&flam), singles 4 r32 & accentN 3 <-> singles 4 r32 & accentN 3 ]
            return2 = twice [singles 6 r32 & accentFirst <-> r16&drag, singles 4 r32&accentFirst <-> singles 4 r32 & accentFirst]

    ending2 = [
      dotCut (r16&flam<-> l16<-> r16&flam<-> l16&flam), singles 4 r32 <-> dotCut (r16&flam<-> l16)
